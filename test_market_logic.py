...
def create_test_market_data(symbol: str = "BTCUSDT", price: float = 45000.0) -> MarketDataMessage:
...
async def test_onchain_metrics_strategy():
    ...
    for symbol, price in test_cases:
        ...
        if signal:
            print(f"  ✅ On-Chain Signal Generated:")
            print(f"    Symbol: {signal.symbol}")
            print(f"    Type: {signal.signal_type}")
            print(f"    Action: {signal.signal_action}")
            print(f"    Confidence: {signal.confidence_score:.2f}")
            ...
            print(f"  ❌ No on-chain signal generated")
