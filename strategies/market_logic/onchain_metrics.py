...
class OnChainMetricsStrategy:
    ...
    def __init__(self, logger: Optional[structlog.BoundLogger] = None):
        ...
        self.network_growth_threshold = constants.ONCHAIN_NETWORK_GROWTH_THRESHOLD  # 10%
        ...
        self.logger.info("On-Chain Metrics Strategy initialized",
                        network_growth_threshold=self.network_growth_threshold,
                        volume_threshold=self.volume_threshold)

    async def process_market_data(self, market_data: MarketDataMessage) -> Optional[Signal]:
        """
        ...
        try:
            ...
            if signal:
                ...
                self.logger.info("On-chain metrics signal generated",
    ...
    async def _analyze_onchain_metrics(self, market_data: MarketDataMessage) -> Optional[Signal]:
        """
    ...
    def _calculate_growth_metrics(self, asset_key: str) -> Optional[Dict[str, float]]:
        ...
        week_ago = history[-168] if len(history) >= 168 else history[0]  # 7 days * 24 hours
        ...
        try:
            ...
            if asset_key == "BTC":
                growth_metrics["active_addresses_24h"] = self._calculate_percentage_change(
                    day_ago.get("active_addresses", 0), current.get("active_addresses", 0)
                )
                growth_metrics["transaction_volume_24h"] = self._calculate_percentage_change(
                    day_ago.get("transaction_volume_btc", 0), current.get("transaction_volume_btc", 0)
                )
                ...
                growth_metrics["net_exchange_flow"] = inflow - outflow  # Positive = net inflow (bearish)
                ...
                growth_metrics["transaction_volume_24h"] = self._calculate_percentage_change(
                    day_ago.get("transaction_volume_eth", 0), current.get("transaction_volume_eth", 0)
                )
    ...
    def _evaluate_fundamental_conditions(self, asset_key: str, current_metrics: Dict[str, Any],
                                       growth_metrics: Dict[str, float], 
                                       market_data: MarketDataMessage) -> Optional[Signal]:
        """
        ...
        if (active_addresses_growth > self.network_growth_threshold and
            transaction_volume_growth > self.volume_threshold):
            
            # Additional confirmation for Bitcoin
            if asset_key == "BTC":
                hash_rate_growth = growth_metrics.get("hash_rate_24h", 0)
                if hash_rate_growth > 0:  # Hash rate increasing = network security improving
                    confidence_score = min(0.8, (active_addresses_growth + transaction_volume_growth) / 30)
            ...
            # Additional confirmation for Ethereum
                ...
                if defi_tvl_growth > 5:  # DeFi TVL growing = ecosystem usage increasing
                    confidence_score = min(0.75, (active_addresses_growth + transaction_volume_growth) / 35)
        ...
        elif net_exchange_flow > 0 and abs(net_exchange_flow) > 1000:  # Significant inflow threshold
    ...
    def _create_onchain_signal(self, signal_type: SignalType, action: SignalAction,
                             symbol: str, confidence_score: float, reasoning: str,
                             market_data: MarketDataMessage, metadata: Dict[str, Any]) -> Signal:
