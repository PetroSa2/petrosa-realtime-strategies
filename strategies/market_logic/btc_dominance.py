...
class BitcoinDominanceStrategy:
    ...
    def __init__(self, logger: Optional[structlog.BoundLogger] = None):
        ...
        self.min_signal_interval = constants.BTC_DOMINANCE_MIN_SIGNAL_INTERVAL  # 4 hours
        ...
        self.logger.info("Bitcoin Dominance Strategy initialized", 
    ...
    async def process_market_data(self, market_data: MarketDataMessage) -> Optional[Signal]:
        ...
        try:
            ...
            signal = await self._generate_dominance_signal(current_dominance, market_data)
            
            if signal:
                self.signals_generated += 1
                self.logger.info("Bitcoin dominance signal generated",
    ...
    async def _calculate_btc_dominance(self) -> Optional[float]:
        ...
        try:
            ...
            eth_momentum = self._calculate_momentum(eth_data, window_start) if eth_data else 0
            bnb_momentum = self._calculate_momentum(bnb_data, window_start) if bnb_data else 0
    ...
    def _calculate_momentum(self, price_data: List[Dict[str, Any]], window_start: float) -> float:
        """Calculate price momentum over the specified window."""
        recent_data = [entry for entry in price_data if entry['timestamp'] >= window_start]
    ...
    def _update_dominance_history(self, dominance: float) -> None:
        ...
        self.dominance_history = [
            entry for entry in self.dominance_history 
            if entry['timestamp'] > cutoff_time
        ]

    async def _generate_dominance_signal(self, current_dominance: float, 
        ...
        if current_dominance > self.high_threshold:
            if dominance_trend == "rising" or dominance_change_24h > self.change_threshold:
        ...
        # Low dominance scenarios (QTZD-style threshold logic)
            ...
            if dominance_trend == "falling" or dominance_change_24h < -self.change_threshold:
    ...
    def _calculate_dominance_change_24h(self) -> float:
        ...
        past_entries = [entry for entry in self.dominance_history 
    ...
    def _create_signal(self, signal_type: SignalType, action: SignalAction, 
