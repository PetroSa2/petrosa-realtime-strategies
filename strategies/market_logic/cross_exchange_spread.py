...
class CrossExchangeSpreadStrategy:
    ...
    def __init__(self, logger: Optional[structlog.BoundLogger] = None):
        ...
        self.logger.info("Cross-Exchange Spread Strategy initialized",
                        spread_threshold=self.spread_threshold,
                        exchanges=self.exchanges)

    async def process_market_data(self, market_data: MarketDataMessage) -> Optional[List[Signal]]:
        """
        ...
        try:
            ...
            if signals:
                ...
                self.logger.info("Cross-exchange arbitrage signals generated",
            ...
            self.logger.error("Error processing cross-exchange spread data", error=str(e))
            return None
    ...
    async def _fetch_exchange_price(self, session: aiohttp.ClientSession, exchange: str) -> None:
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        if 'data' in data and 'rates' in data['data']:
                            ...
                            if btc_price > 0:
                                self.price_cache[f"coinbase_BTCUSDT"] = {
                                    'price': btc_price,
                                    'timestamp': current_time,
                                    'exchange': 'coinbase',
                                    'symbol': 'BTCUSDT'
                    ...
                    if response.status == 200:
                        ...
                        if 'result' in data and 'XXBTZUSD' in data['result']:
                            ...
                            if 'c' in ticker_data and len(ticker_data['c']) > 0:
                                ...
                                self.price_cache[f"kraken_BTCUSDT"] = {
                                    'price': btc_price,
                                    'timestamp': current_time,
                                    'exchange': 'kraken',
                                    'symbol': 'BTCUSDT'
    ...
    async def _generate_spread_signals(self, market_data: MarketDataMessage) -> Optional[List[Signal]]:
        """
        ...
        highest_exchange = max(exchange_prices, key=lambda x: exchange_prices[x]['price'])
        lowest_exchange = min(exchange_prices, key=lambda x: exchange_prices[x]['price'])
        ...
        self._update_spread_history(symbol, spread_percent, highest_exchange, lowest_exchange)
        ...
        if spread_percent >= self.spread_threshold:
            ...
            if self._should_generate_signal(signal_key):
                ...
                confidence_score = min(0.95, spread_percent / 2.0)  # Higher spread = higher confidence
                ...
                buy_signal = self._create_arbitrage_signal(
                    ...
                    metadata={
                        ...
                        "position_size_usdt": min(self.max_position_size, 
    ...
    def _get_exchange_prices(self, symbol: str) -> Dict[str, Dict[str, Any]]:
        ...
        for cache_key, price_data in self.price_cache.items():
            if (price_data['symbol'] == symbol and 
                current_time - price_data['timestamp'] <= max_age):
                exchange_prices[price_data['exchange']] = price_data
                
        return exchange_prices

    def _update_spread_history(self, symbol: str, spread_percent: float,
                             highest_exchange: str, lowest_exchange: str) -> None:
        ...
        self.spread_history[symbol] = [
            entry for entry in self.spread_history[symbol]
    ...
    def _create_arbitrage_signal(self, signal_type: SignalType, action: SignalAction,
                               symbol: str, confidence_score: float, reasoning: str,
                               market_data: MarketDataMessage, exchange: str, 
                               price: float, metadata: Dict[str, Any]) -> Signal:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_exchanges": list(set(
                data['exchange'] for data in self.price_cache.values()
            )),
