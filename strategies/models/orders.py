...
class TradeOrder(BaseModel):
    ...
    signal_id: str = Field(..., description="ID of the signal that triggered this order")
    confidence_score: float = Field(..., ge=0.0, le=1.0, description="Signal confidence score")
    ...
    def validate_stop_price_for_stop_orders(cls, v, values):
        ...
        if values.get('order_type') in [OrderType.STOP_MARKET, OrderType.STOP_LIMIT] and v is None:
...
class OrderMetrics(BaseModel):
    ...
    orders_by_strategy: Dict[str, int] = Field(default_factory=dict, description="Orders by strategy")
    orders_by_symbol: Dict[str, int] = Field(default_factory=dict, description="Orders by symbol")
    ...
    last_order_timestamp: Optional[datetime] = Field(default=None, description="Last order timestamp")
    ...
    def update_metrics(self, order: TradeOrder, response: OrderResponse, processing_time_ms: float) -> None:
        ...
        self.orders_by_strategy[order.strategy_name] = self.orders_by_strategy.get(order.strategy_name, 0) + 1
        
        # Update orders by symbol
        self.orders_by_symbol[order.symbol] = self.orders_by_symbol.get(order.symbol, 0) + 1
        ...
        if self.total_orders_submitted == 1:
            ...
            self.average_processing_time_ms = (
                (self.average_processing_time_ms * (self.total_orders_submitted - 1) + processing_time_ms) 
        ...
        successful_orders = sum(
            count for status, count in self.orders_by_status.items() 
