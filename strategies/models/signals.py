...
class StrategySignal(BaseModel):
    ...
    input_data: Dict[str, Any] = Field(default_factory=dict, description="Input data used by strategy")
    processing_time_ms: float = Field(..., ge=0, description="Processing time in milliseconds")
    strategy_specific_metrics: Dict[str, float] = Field(default_factory=dict, description="Strategy-specific metrics")
...
class SignalAggregation(BaseModel):
    ...
    symbol: str = Field(..., description="Trading symbol")
    aggregated_signal_type: SignalType = Field(..., description="Aggregated signal type")
    aggregated_signal_action: SignalAction = Field(..., description="Aggregated signal action")
    aggregated_confidence_score: float = Field(..., ge=0.0, le=1.0, description="Aggregated confidence score")
    aggregated_confidence: SignalConfidence = Field(..., description="Aggregated confidence level")
    strategy_signals: Dict[str, StrategySignal] = Field(..., description="Individual strategy signals")
    aggregation_method: str = Field(..., description="Method used for aggregation")
    aggregation_weights: Dict[str, float] = Field(default_factory=dict, description="Weights used for aggregation")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Aggregation timestamp")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
    ...
    def is_strong_consensus(self) -> bool:
        ...
        consensus_count = sum(
            1 for signal in self.strategy_signals.values() 
            if signal.signal_type == consensus_type
        )
        
        return consensus_count >= len(self.strategy_signals) * 0.7  # 70% consensus threshold
...
class SignalMetrics(BaseModel):
    ...
    signals_by_type: Dict[SignalType, int] = Field(default_factory=dict, description="Signals by type")
    signals_by_confidence: Dict[SignalConfidence, int] = Field(default_factory=dict, description="Signals by confidence")
    signals_by_strategy: Dict[str, int] = Field(default_factory=dict, description="Signals by strategy")
    ...
    def update_metrics(self, signal: Signal, processing_time_ms: float) -> None:
        ...
        self.signals_by_type[signal.signal_type] = self.signals_by_type.get(signal.signal_type, 0) + 1
        
        # Update signals by confidence
        self.signals_by_confidence[signal.confidence] = self.signals_by_confidence.get(signal.confidence, 0) + 1
        
        # Update signals by strategy
        self.signals_by_strategy[signal.strategy_name] = self.signals_by_strategy.get(signal.strategy_name, 0) + 1
        ...
        if self.total_signals_generated == 1:
            ...
            self.average_processing_time_ms = (
                (self.average_processing_time_ms * (self.total_signals_generated - 1) + processing_time_ms) 
    ...
    def get_signal_distribution(self) -> Dict[str, float]:
        ...
        for signal_type, count in self.signals_by_type.items():
            distribution[f"type_{signal_type.value.lower()}"] = count / self.total_signals_generated
        
        # Confidence distribution
        for confidence, count in self.signals_by_confidence.items():
            distribution[f"confidence_{confidence.value.lower()}"] = count / self.total_signals_generated
