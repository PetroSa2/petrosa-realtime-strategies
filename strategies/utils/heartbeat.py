...
class HeartbeatManager:
    ...
    def __init__(
        ...
        self.interval_seconds = interval_seconds if interval_seconds is not None else constants.HEARTBEAT_INTERVAL_SECONDS
        self.include_detailed_stats = include_detailed_stats if include_detailed_stats is not None else constants.HEARTBEAT_INCLUDE_DETAILED_STATS
    ...
    def _calculate_deltas(self, current_stats: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate deltas since last heartbeat."""
        return {
            "consumer_messages": current_stats["consumer_messages"] - self.previous_stats["consumer_messages"],
            "consumer_errors": current_stats["consumer_errors"] - self.previous_stats["consumer_errors"],
            "publisher_orders": current_stats["publisher_orders"] - self.previous_stats["publisher_orders"],
            "publisher_errors": current_stats["publisher_errors"] - self.previous_stats["publisher_errors"],
        }
    ...
    def _calculate_rates(self, delta_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "messages_per_second": round(delta_stats["consumer_messages"] / self.interval_seconds, 2),
            "orders_per_second": round(delta_stats["publisher_orders"] / self.interval_seconds, 2),
            "error_rate_per_second": round((delta_stats["consumer_errors"] + delta_stats["publisher_errors"]) / self.interval_seconds, 2),
        }
    ...
    def _collect_detailed_stats(self) -> Dict[str, Any]:
        ...
        if self.consumer:
            try:
                ...
                detailed.update({
                    "consumer_is_running": consumer_metrics.get("is_running", False),
                    "consumer_is_healthy": consumer_health.get("healthy", False),
                    "consumer_nats_connected": consumer_health.get("nats_connected", False),
                    "consumer_subscription_active": consumer_health.get("subscription_active", False),
                    "consumer_last_message_time": consumer_metrics.get("last_message_time"),
                    "consumer_avg_processing_time_ms": round(consumer_metrics.get("avg_processing_time_ms", 0), 2),
                    "consumer_max_processing_time_ms": round(consumer_metrics.get("max_processing_time_ms", 0), 2),
                    "consumer_circuit_breaker_state": consumer_metrics.get("circuit_breaker_state", "unknown"),
                })
            except Exception as e:
                self.logger.warning("Failed to get detailed consumer stats", error=str(e))
        ...
        if self.publisher:
            try:
                ...
                detailed.update({
                    "publisher_is_running": publisher_metrics.get("is_running", False),
                    "publisher_is_healthy": publisher_health.get("healthy", False),
                    "publisher_nats_connected": publisher_health.get("nats_connected", False),
                    "publisher_queue_size": publisher_metrics.get("queue_size", 0),
                    "publisher_last_order_time": publisher_metrics.get("last_order_time"),
                    "publisher_avg_publishing_time_ms": round(publisher_metrics.get("avg_publishing_time_ms", 0), 2),
                    "publisher_max_publishing_time_ms": round(publisher_metrics.get("max_publishing_time_ms", 0), 2),
                    "publisher_circuit_breaker_state": publisher_metrics.get("circuit_breaker_state", "unknown"),
                })
            except Exception as e:
                self.logger.warning("Failed to get detailed publisher stats", error=str(e))
