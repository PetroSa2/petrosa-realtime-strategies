...
def health_server(mock_components):
    ...
    with patch("strategies.health.server.constants") as mock_constants:
        ...
        mock_constants.get_strategy_config.return_value = {"strategy1": {"param": "value"}}
...
def test_healthz_endpoint_healthy(client):
    ...
    health_server = client.app.state.health_server if hasattr(client.app, 'state') else None
    if health_server:
...
def test_metrics_endpoint_error(client):
    ...
    with patch("strategies.health.server.generate_latest", side_effect=Exception("Metrics error")):
        response = client.get("/metrics")
...
def test_info_endpoint_error(client):
    ...
    with patch("strategies.health.server.constants.get_enabled_strategies", side_effect=Exception("Config error")):
        response = client.get("/info")
        assert response.status_code == 500
...
async def test_start_error_handling(health_server):
    ...
    with patch("strategies.health.server.uvicorn.Server", side_effect=Exception("Start error")):
...
def test_prometheus_metrics_generation(health_server):
    ...
    with patch("strategies.health.server.generate_latest", return_value=b"# HELP test_metric Test metric\n# TYPE test_metric counter\ntest_metric 1\n"):
        with patch.object(health_server, '_get_memory_usage', return_value=100.0):
            with patch.object(health_server, '_get_cpu_usage', return_value=25.0):
