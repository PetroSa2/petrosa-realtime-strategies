...
from strategies.services.depth_analyzer import (
    DepthAnalyzer,
    DepthMetrics,
    MarketPressureHistory,
)
...
def analyzer():
    ...
    return DepthAnalyzer(
        history_window_seconds=900, max_symbols=100, metrics_ttl_seconds=300
    )
...
def test_analyze_depth_pressure_calculation(analyzer, sample_bids, sample_asks):
    ...
    assert (
        abs(metrics.net_pressure - (expected_buy_pressure - expected_sell_pressure))
        < 0.1
    )
...
def test_cleanup_expired_metrics(analyzer, sample_bids, sample_asks):
    ...
    with patch(
        "strategies.services.depth_analyzer.time.time", return_value=time.time()
    ):
