...
def test_heartbeat_with_consumer_and_publisher():
    ...
    mock_consumer = Mock()
    mock_consumer.get_statistics = Mock(
        return_value={"messages_processed": 100, "errors": 5}
    )

    mock_publisher = Mock()
    mock_publisher.get_metrics = Mock(
        return_value={"order_count": 50, "error_count": 2}
    )

    manager = HeartbeatManager(
        consumer=mock_consumer, publisher=mock_publisher, enabled=True
    )
