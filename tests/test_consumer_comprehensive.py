...
from strategies.models.market_data import MarketDataMessage, TickerData, TradeData, DepthUpdate
...
async def test_start_connection_failure(consumer):
    ...
    with patch("strategies.core.consumer.nats.connect", side_effect=Exception("Connection failed")):
...
@pytest.mark.asyncio
async def test_subscribe_to_topic_success(consumer, mock_nats_client, mock_subscription):
...
async def test_process_message_success(consumer):
    ...
    with patch.object(consumer, '_parse_market_data', return_value=MarketDataMessage(
        stream="btcusdt@ticker",
        data=TickerData(
            symbol="BTCUSDT",
            price=50000.00,
...
async def test_process_message_routing_error(consumer):
    ...
    with patch.object(consumer, '_parse_market_data', return_value=MarketDataMessage(
        stream="btcusdt@ticker",
        data=TickerData(symbol="BTCUSDT", price=50000.00, volume=0, timestamp=datetime.utcnow())
    )):
        with patch.object(consumer, '_route_to_strategies', side_effect=Exception("Routing error")):
            await consumer._process_message(json.dumps(message_data).encode())
...
async def test_route_to_strategies_ticker(consumer):
    ...
    message = MarketDataMessage(
        stream="btcusdt@ticker",
        data=TickerData(symbol="BTCUSDT", price=50000.00, volume=100.0, timestamp=datetime.utcnow())
    )
    
    with patch.object(consumer.market_logic_strategies["btc_dominance"], 'process_ticker', return_value=None):
        await consumer._route_to_strategies(message)
        
        # Verify strategy was called
        consumer.market_logic_strategies["btc_dominance"].process_ticker.assert_called_once()
...
async def test_route_to_strategies_trade(consumer):
    ...
    message = MarketDataMessage(
        stream="btcusdt@trade",
        data=TradeData(symbol="BTCUSDT", price=50000.00, quantity=0.1, timestamp=datetime.utcnow())
    )
    
    with patch.object(consumer.market_logic_strategies["btc_dominance"], 'process_trade', return_value=None):
        await consumer._route_to_strategies(message)
        
        consumer.market_logic_strategies["btc_dominance"].process_trade.assert_called_once()
...
async def test_route_to_strategies_depth(consumer):
    ...
    with patch.object(consumer.microstructure_strategies["spread_liquidity"], 'process_depth', return_value=None):
        await consumer._route_to_strategies(message)
        
        consumer.microstructure_strategies["spread_liquidity"].process_depth.assert_called_once()
...
async def test_route_to_strategies_strategy_error(consumer):
    ...
    with patch.object(consumer.market_logic_strategies["btc_dominance"], 'process_ticker', side_effect=Exception("Strategy error")):
...
async def test_circuit_breaker_integration(consumer):
    ...
    with patch.object(consumer.circuit_breaker, 'call', side_effect=Exception("Circuit breaker open")):
...
async def test_trace_context_extraction(consumer):
    ...
    with patch("strategies.core.consumer.extract_trace_context", return_value={"trace_id": "123"}) as mock_extract:
        with patch.object(consumer, '_route_to_strategies', return_value=None):
            await consumer._process_message(json.dumps(message_data).encode())
