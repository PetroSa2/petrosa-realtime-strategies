...
class PipelineRunner:
    ...
    def run_command(self, command: List[str], description: str, 
        ...
        try:
            ...
            error_msg = f"{description} failed with exit code {e.returncode}: {e.stderr}"
            self.log(f"❌ {error_msg}", "error")
            self.errors.append(error_msg)
            return False, error_msg
    ...
    def check_prerequisites(self) -> bool:
        ...
        for name, command in prerequisites:
            success, output = self.run_command(command, f"Checking {name}", capture_output=True, check=False)
            if not success:
                all_good = False
...
def main():
    parser = argparse.ArgumentParser(description="Petrosa Realtime Strategies Pipeline Runner")
    parser.add_argument("--stages", nargs="+", 
                       choices=["prerequisites", "setup", "linting", "tests", "security", "docker", "container", "deploy"],
