...
# Default target
help:
	@echo "🚀 Petrosa Realtime Strategies - Standardized Development Commands"
	@echo "================================================================"
	@echo ""
	...
	@echo "  run-local      - Run strategies service locally"
...
container:
	...
	docker run --rm petrosa-realtime-strategies:latest --help

docker-clean:
	@echo "🧹 Cleaning up Docker images..."
	docker rmi petrosa-realtime-strategies:latest 2>/dev/null || true
...
k8s-status:
	...
	kubectl --kubeconfig=k8s/kubeconfig.yaml get svc -n petrosa-apps -l app=realtime-strategies

k8s-logs:
	@echo "📋 Kubernetes logs:"
	kubectl --kubeconfig=k8s/kubeconfig.yaml logs -n petrosa-apps -l app=realtime-strategies --tail=50
...
# Local development
run-local:
	@echo "🚀 Running strategies service locally..."
